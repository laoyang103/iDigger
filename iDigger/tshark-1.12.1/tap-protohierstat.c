...
static void
phs_draw_json(phs_t *rs, cJSON *to_fill)
{
    cJSON *child = NULL;
	for(;rs;rs=rs->sibling){
		if(rs->protocol==-1) return;
        child = cJSON_CreateObject();
        cJSON_AddItemToObject(child, "bytes", cJSON_CreateNumber(rs->bytes));
        cJSON_AddItemToObject(child, "frames", cJSON_CreateNumber(rs->frames));
        cJSON_AddItemToObject(to_fill, rs->proto_name, child);
		phs_draw_json(rs->child, child);
	}
}
...
protohierstat_draw(void *prs)
{
	...
	phs_draw_json(rs, out_json);
    str = cJSON_PrintUnformatted(out_json);
	printf("%s\n", str);
    free(str);
